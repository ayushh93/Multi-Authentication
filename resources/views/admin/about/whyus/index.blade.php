                    ...
                    <div class="card">
                        <div class="card-header d-flex justify-content-between">
                            ...
                            <a class="btn btn-primary cart-btn-transform pull-right"
                                href="{{ route('admin.about.whyus.create') }}">Add Why us</a>
                        </div>
                                ...
                                <table class="display" id="data-source-1" style="width:100%">
                                    ...
                                    <tbody>
                                        @isset($whyus)
                                            ...
                                            <tr>
                                                <td>{{ $loop->index + 1 }}</td>
                                                <td> <img src="{{ asset('uploads/about/whyus/' . $item->image) }}" width="70px"
                                                        height="50px"></td>
                                                <td>{{ $item->title }}</td>
                                                <td>{{ $item->excerpt }}</td>
                                                <td class="d-flex"> {{-- edit data --}}
                                                    <a href="{{ route('admin.about.whyus.edit', $item->id) }}">
                                                        <button class="btn btn-success btn-sm" style="margin-left: 2px">
                                                            <i class="fa fa-pencil"></i>
                                                        </button>
                                                    </a>
                                                    {{-- delete data --}}
                                                    <form action="{{ route('admin.about.whyus.destroy', $item->id) }}"
                                                        method="POST">
                                                        @csrf
                                                        @method('DELETE')
